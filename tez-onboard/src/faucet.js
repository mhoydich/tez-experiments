...
//      Shadownet faucet once (link shown in UI), then your faucet tops them up
//   B) run a relayer: your server (holding the relayer key) calls the
//      contract's `claim_for(address)` entrypoint. For Shadownet demos, (A) is fine.
...
export async function maybeSeed(Tezos, address, balanceTez, statusEl) {
  ...
  if (balanceTez === 0) {
    statusEl.innerHTML =
      ...
      `<a href="https://faucet.shadownet.teztnets.com" target="_blank" rel="noopener">Shadownet faucet</a> ` +
